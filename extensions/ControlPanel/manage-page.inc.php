...
if($database instanceof Database &&!array_key_exists("created", $fields)
        && !array_key_exists("created-by", $fields) && !array_key_exists("modified", $fields)
        && !array_key_exists("modified-by", $fields)) {
...
if(count($query['results']))
	foreach($query['results'] as $entry){
		...
		foreach($fields as $field => $data){
			...
			else if(isset($data['render-html']))
				$value = call_user_func($data['render-html'], $entry);
			else if(isset($data['render-html-field']))
		    ...
		    else if(isset($data['value-html']))
				$value = nl2br(interpolate($data['value-html'], false, array_merge($entry, Array(
																"b" => "<strong>", "endb" => "</strong>",
																"small" => "<span style=\"font-size: 10px\">",
																"endsmall" => "</span>"
															))));
			else if(is_array($data) && isset($data['render-adv']))
