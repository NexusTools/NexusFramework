...
class ControlPanel {
	    ...
	    if(!$condition) {
	        ...
	        switch($default){
	            case self::BLACKLIST_NAVIGATION:
	                ...
	                return isset(self::$naventries[$topLayer][$section]) &&
	                            !self::isPageVisible($section, $page);
	...
	public static function registerPage($cat, $name, $script, $addToNav=true, $catLayer=1, $topLayer=1, $asPopup=false){
	    ...
	    if($addToNav)
	        if(!self::canUserAccessPage($cat, $name))
	            $addToNav = false;
	        else if(!isset(self::$naventries[$topLayer][$cat]))
	...
	public static function has($cat, $page=false){
	    return isset(self::$categories[$cat]) && (!$page
	        || isset(self::$categories[$cat]['assoc'][$page]));
	}
