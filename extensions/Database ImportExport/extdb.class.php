<?php
class DatabaseExtensionDatabase extends BasicEmuDatabase {

	private static $instance;

	public static function getInstance() {
		return self::$instance ? self::$instance : (self::$instance = new DatabaseExtensionDatabase());
	}
		...
		while (($ex = readdir($dir)) !== false) {
			...
			if (is_file(($infoFile = "$dirName$ex".DIRSEP."info.json"))) {
				...
				if (!isset($infoData['database']))
					continue;
				if (!ExtensionLoader::isExtensionLoaded($infoData['name']))
					continue;
