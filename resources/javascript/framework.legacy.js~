var Framework = {
	init: function() {
		...
		try {
			Framework.Config = $$("div.framework_config")[0].innerHTML;
		...
		Framework.PageElement = $$("div.framework_page")[0];
	...
	API: {
		...
		unregisterHandler: function(module, callback){
		},
		...
		request: function(module, data, postVars){
			...
			if(Framework.API.requestTimeout == null)
				Framework.API.requestTimeout = setTimeout(Framework.API.makeRequests, 100);
		},
				...
				if(e.target.status == 200){
					...
					for(module in data){
						if(Framework.API.callbacks[module] &&
							...
							Framework.API.callbacks[module](data[module]);
					}
				}
			}
		}
