	...
	if (startsWith($path, FRAMEWORK_PATH))
		...
		if (startsWith($path, INDEX_PATH))
			...
			if (startsWith($path, MEDIA_PATH))
				...
				if (startsWith($path, BASE_TMP_PATH))
...
function fullpath($path) {
	...
	if (startsWith($path, INDEX_PATH) || startsWith($path, BASE_TMP_PATH) || startsWith($path, MEDIA_PATH) || startsWith($path, FRAMEWORK_PATH) || startsWith($path, Theme::getPath()))
		return properpath($path);
	else
		if (startsWith($path, ":".DIRSEP))
	...
	if (isset($__framework_activePath) && file_exists($rpath = $__framework_activePath.DIRSEP.$path))
		return properpath($rpath);
...
function is_assoc($array) {
	return ($object instanceof Traversable) || (is_array($array) && (count($array) == 0 || 0 !== count(array_diff_key($array, array_keys(array_keys($array))))));
}
