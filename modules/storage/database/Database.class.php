...
class Database extends Lockable {
	...
	public function getDefinition($table = null) {
		...
		if (is_string($table)) {
			...
			$fieldDefs = array_key_exists("fields", $this->databaseDefinition[$table]) ? $this->databaseDefinition[$table]['fields'] : $this->databaseDefinition[$table];
			...
			foreach ($fieldDefs as & $keyDef) {
				if (is_string($keyDef) && startsWith($keyDef, ":")) {
					...
					if (is_array($keyDef) && startsWith($keyDef['type'], ":")) {
	...
	public static function timeToTimestamp($time = 0, $inputTZ ="UTC") {
		...
		return DateFormat::formatSqlTimestamp($time, $inputTZ);
	}
	...
	public function _selectField($table, $where, $field, $default = false, $orderBy = false, $resolveProtoValue = false) {
		...
		if ($res && array_key_exists(0, $res) && array_key_exists(0, $res[0])) {
			$value = $res[0][0];
	...
	public function _insert($table, $values) {
		...
		foreach ($tblDef as $field => $def)
			if (is_array($def) && array_key_exists('default', $def) && preg_match("/^{{.+}}$/", $def['default']) && !array_key_exists($field, $values))
	...
	public function _update($table, $values, $where = false) {
		...
		$tblDef = $this->getDefinition($table);
		foreach ($tblDef as $field => $def)
			if (is_array($def) && array_key_exists('update', $def) && !array_key_exists($field, $values))
	...
	protected function _createOrUpgradeTable($name, $def) {
		...
		foreach ($tableFields as $key => $keyDef) {
			...
			$numericType = $keyDef['type'] == "INTEGER" || $keyDef['type'] == "BOOLEAN" || $keyDef['type'] == "TINYINT" || $keyDef['type'] == "SMALLINT" || $keyDef['type'] == "LARGEINT";
			...
			if (array_key_exists('default', $keyDef)) {
				...
				if (is_bool($default))
					...
					if (preg_match("/^{{.+}}$/", $default))
						...
						if (!is_numeric($default) && !in_array($default, self::$knownKeywords))
	...
	private function __construct($extension, $def, $name = "database") {
		...
		try {
			...
			// Attempt Upgrade
			if ($name == "database" && !is_file($this->dbPath) && is_file(CONFIG_PATH."$extension.definition.json") && is_file(CONFIG_PATH."$extension.sqlite")) {
				if (!copy(CONFIG_PATH."$extension.definition.json", $this->defPath) || !copy(CONFIG_PATH."$extension.sqlite", $this->dbPath))
	...
	public static function getInstance($extension = null, $definition = null, $name = "database") {
		if (!$extension) { // Detect Name from Paths
			...
			foreach ($backtrace as $frame) {
				if (!isset($frame['file']) || (startsWith($frame['file'], FRAMEWORK_PATH) && !startsWith($frame['file'], FRAMEWORK_EXT_PATH)))
